...
class NumpyFrameBuffer:
    ...
    def fill_triangle(
        ...
        max_x = min(self.width - 1, int(np.ceil(max(a[0], b[0], c[0]))))
        min_y = max(0, int(np.floor(min(a[1], b[1], c[1]))))
        max_y = min(self.height - 1, int(np.ceil(max(a[1], b[1], c[1]))))
        ...
        px = np.arange(min_x, max_x + 1, dtype=np.float32)[None, :] + 0.5
        py = np.arange(min_y, max_y + 1, dtype=np.float32)[:, None] + 0.5
